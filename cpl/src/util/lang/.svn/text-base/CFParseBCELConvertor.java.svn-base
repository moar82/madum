		...
		for (int index = 0; index < methods.length; index++) {
			...
			if (method.getAccessFlags() > 0) {
				methodSignature.append(
					Modifier.toString(method.getAccessFlags()));
				methodSignature.append(' ');
			}
			...
			final StringBuffer buffer =
				new StringBuffer(methodInfo.getDesc());
			buffer.insert(buffer.lastIndexOf(")"), ")");
			((ConstantPool.Utf8Entry) cp.get(idxDescriptor)).setValue(
				buffer.toString());
			...
			if (method.getCode() != null) {
				...
				codeAttributeInfo.setMaxLocals(
				...
				if (method.getLineNumberTable() != null) {
					final LineNumberAttrInfo lineNumberAttrInfo =
						(LineNumberAttrInfo) codeAttributeInfo
							.getAttrs()
							.add(
							"LineNumberTable");
					try {
		...
		for (int index = 1; index < constants.length; index++) {
			...
			if (constant instanceof ConstantClass) {
				...
				final ConstantUtf8 classNameUtf8 =
					(ConstantUtf8) bcelCP.getConstant(
						classConstant.getNameIndex());

				final int nameAndTypeIndex =
					((ConstantFieldref) constant).getNameAndTypeIndex();
				final ConstantNameAndType nameAndTypeConstant =
					(ConstantNameAndType) bcelCP.getConstant(
						nameAndTypeIndex);
				...
				cfparseCP.addString(
					(String) ((ConstantString) constant).getConstantValue(
						bcelCP));
			}
