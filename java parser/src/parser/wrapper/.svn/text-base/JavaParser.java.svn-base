...
public class JavaParser {
	...
	private List<NamedCompilationUnit> namedCompilationUnitContainer;
	...
	public JavaParser(final SourceInputsHolder javaProject) {
		...
		this.namedCompilationUnitContainer=new ArrayList<NamedCompilationUnit>();
	}
...
//	}
	 ...
	 */
	public void parse(final ExtendedASTVisitor visitor) {
		if (this.namedCompilationUnitContainer.isEmpty()) {
			for (final NamedReader compilationUnit : this.javaProject
				.getCompilationUnitList()) {
				...
				if (astNode instanceof CompilationUnit) {
					...
					// namedCU.accept(visitor);
					this.namedCompilationUnitContainer.add(namedCU);
				}
			}
		}
		
			for (final NamedCompilationUnit currentNamedCU : this.namedCompilationUnitContainer) {
				currentNamedCU.accept(visitor);
			}
	}
}
