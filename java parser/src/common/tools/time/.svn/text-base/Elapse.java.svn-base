...
public class Elapse implements Comparable<Elapse> {
	...
	public Elapse(final Date start, final Date end) {
		...
		this.hours = (int) (this.elapseInMS % Constants.DAY_IN_MS / Constants.HOUR_IN_MS);
		this.minutes = (int) (this.elapseInMS % Constants.HOUR_IN_MS / Constants.MINUTE_IN_MS);
		this.seconds = (int) (this.elapseInMS % Constants.MINUTE_IN_MS / 1000);
		this.mSecond = (int) (this.elapseInMS % 1000);

		buffer.append(this.days).append(" days ").append(this.hours)
				.append(" hours ").append(this.minutes).append(" minutes ")
				.append(this.seconds).append(" seconds ").append(this.mSecond)
				.append(" ms");
