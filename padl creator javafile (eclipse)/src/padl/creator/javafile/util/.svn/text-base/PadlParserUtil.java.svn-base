...
public class PadlParserUtil {
	...
	public static IFirstClassEntity createGhost(
		...
		try {
			...
			Output.getInstance().normalOutput().println(aFullyQualifiedName);
			return anAbstractLevelModel
				.getTopLevelEntityFromID(aFullyQualifiedName);
		}
	}
	...
	private static IPackage createPackage(
		...
		if (packaje == null) {
			try {
				packaje =
					aCodeLevelModel.getFactory().createPackage(aPackageName);
	...
	public static List<IParameter> getListOfParameters(
		final List<SingleVariableDeclaration> aListOfVars,
		final IPackage packag,
		final ICodeLevelModel model) {
		// get the list of parameters

		final List<IParameter> listOfParameters = new ArrayList<IParameter>();
		for (final SingleVariableDeclaration var : aListOfVars) {
			if (var.resolveBinding() != null) {
				try {
					...
					if (entity != null) {
						...
						// Aminata 05/05/11
						parameter.setComment(Integer.toString(type
							.getDimensions()));
	...
	private static IFirstClassEntity getOrCreateGhostEntity(
		...
		if (indexOfDot < 0) {
			...
			if (ghost == null) {
				ghost =
					PadlParserUtil.createGhost(aPadlModel, fullyQualifiedName
						.toCharArray());
			}
			return ghost;
		}
			...
			int nbTokens; // number of entities in the
			...
			if (searchedPackage != null) {
				...
				if (currentEntity == null) {
					currentEntity =
						PadlParserUtil.createGhost(aPadlModel, currentEntityID
							.toCharArray());
				}
			}
			else {
